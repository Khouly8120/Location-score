...
function generateDetailedAnalysis(clinic) {
    ...
    Object.keys(SCORING_CONFIG.categories).forEach(categoryKey => {
        ...
        Object.keys(category.metrics).forEach(metricKey => {
            ...
            // CORRECTED: Use benchmark data from Google Sheets for targets
            const targetValue = benchmarkData[categoryKey] ? 
                benchmarkData[categoryKey][metricKey] || metric.target : metric.target;
            
            // Create a metric object with the correct target for calculation
            const metricForCalculation = {
                ...metric,
                target: targetValue
            };
            
            const normalizedScore = normalizeScore(rawScore, metricForCalculation);
            
            // Debug logging for detailed analysis
            console.log(`🔍 Detailed Analysis - ${metric.name}:`, {
                rawScore: rawScore,
                target: targetValue,
                targetSource: benchmarkData[categoryKey] ? 'Google Sheets Benchmark' : 'Config Default',
                ...
                <div style="border: 1px solid #e9ecef; padding: 12px; border-radius: 6px; background: white;">
                    ...
                    <div style="margin-bottom: 8px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px;">
                            ...
                            <span style="font-size: 0.9em; color: #28a745;">${targetValue}${metric.unit}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 4px;">
