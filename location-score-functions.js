...
function populateMonthFilter() {
    ...
    const keys = Object.keys(monthClinicMap).sort((a, b) => b.localeCompare(a)); // newest first
...
function applyMultiMonthAveraging(setting) {
    ...
    clinicNames.forEach(name => {
        ...
        categoryKeys.forEach(catKey => {
            ...
            avgCategories[catKey] = vals.length ? Math.round(vals.reduce((s, v) => s + v, 0) / vals.length) : 0;
        });
